+function ($) {
    ...
    FlatOS.Input.Mouse.prototype.triggerButtonDown = function (id, el) {
        $(el).trigger('mousedown.flatos.'+id);
    };
    ...
    FlatOS.Input.Mouse.prototype.triggerButtonUp = function (id, el) {
        $(el).trigger('mouseup.flatos.'+id);
    };
    ...
    FlatOS.Input.Mouse.prototype.triggerLeftClick = function (id, el) {
        $(el).trigger('click.flatos.'+id);
    };
    ...
    FlatOS.Input.Mouse.prototype.triggerRightClick = function (id, el) {
        $(el).trigger('contextmenu.flatos.'+id);
    };
    ...
    FlatOS.Input.Mouse.prototype.triggerDoubleClick = function (id, el) {
        $(el).trigger('dblclick.flatos.'+id);
    };

    // TODO: Add a submenu handler...
    FlatOS.Input.Mouse.prototype.contextualMenu = function (el, menu) {
            ...
            for (var group in menu) {
                ...
                for (var m in menu[group]) {
                    ...
                    if ($.isFunction(menu[group][m].callback)) {
                        $li.mouseup(menu[group][m].callback);
                    }
                    $li.appendTo($ul);
                }
        ...
        this.rightClick('contextualMenu', el, function (e) {
            ...
            setTimeout(function () { // Delay for Mozilla
                $(document).one('mouseup', function () {that.element.detach();});
            }, 0);
