+function ($) {
        ...
        new FlatOS.Layout('connect-user.html', function(html) {
            ...
            var authenticate = function(e) {
                ...
                if (password === passwd[username]) {
                    ...
                    User.setSessionValue('interface', config.interface);
                    User.setSessionValue('icons', config.icons);
                    User.setSessionValue('username', username);
                    FS.newAlias('~', 'users/' + username);
            ...
            for (var user in users) {
                new FlatOS.Layout('user-item.html', function(item) {
                    var $item  = $(item);
                    var config = new FlatOS.System.UserConfig(user).getConfig('account');
                    var avatar = User.getExternalUserPath(config.acc_picture, user);
    ...
    FlatOS.Interface.prototype.reload = function(ui, callback) {
        ...
        setTimeout(function () {
            ...
            var FS    = new FlatOS.System.FS();
            var UICSS = FS.readDir("system/ui/"+ui+"/interface", true, {'file_type': 'css', 'path_type' : FS.EXTERNAL_PATH});
            var UIJS  = FS.readDir("system/ui/"+ui+"/interface", true, {'file_type': 'js', 'path_type' : FS.EXTERNAL_PATH});
            ...
            for (var css in UICSS) {
                $('<link/>').addClass('user-css').attr({rel: 'stylesheet', type: 'text/css', href: UICSS[css]}).appendTo('head');
            }
