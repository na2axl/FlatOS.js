...
(function($) {
    ...
    var openFolder = function(path, refresh) {
        ...
        if (typeof folders_cache[path] === 'undefined' || refresh == true) {
            ...
            var path_info = new FlatOS.System.File(path);
            var folder = new FlatOS.Api.File(function(file_list) {
                var $ul = $('<ul class="icons medium"></ul>');
                delete file_list['..'];
                for ( var file in file_list ) {
                    var icon = new FlatOS.UI.Icon({path: file_list[file].path, dblClick: false});
                    ...
                    $li
                        .dblclick(function() {
                            var filename = new FlatOS.System.File($(this).attr('data-internal-path'));
                            if (filename.isDir()) {
                                openFolder($(this).attr('data-internal-path'));
                            }
                            else {
                                app = new FlatOS.System.Application.DefaultAppsManager();
    ...
    var User = new FlatOS.Api.User(function(file_list) {
