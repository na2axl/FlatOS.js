(function ($) {
    ...
    const THIS_PROCESS_NAME = 'flatos_codemaster';

    var _window = new FlatOS.Window(THIS_PROCESS_NAME),
    ...
    var exts = {
        'abap': 'abap', 'abc': 'abc', 'as': 'actionscript', 'ada': 'ada', 'asm': 'assembly_x86', 'conf': 'apache_conf', 'htaccess': 'apache_conf',
        'sh': 'sh',
        ...
        $panels.hide(0, function () {
            $(panel_id).show(0, function () {
                ...
                if (typeof file_info[currPanel] === 'undefined') {
                    ...
                    if (typeof file_info[currPanel].externalPath !== 'undefined') {
                        _window.setTitle(file_info[currPanel].externalPath + ' - CodeMaster');
                    }
                    else {
                        _window.setTitle('CodeMaster');
                    }
    ...
    var add_panel = function (panel_title) {
        ...
        editor[currPanel].getSession().on("change", function () {
            ...
            if (typeof file_info[currPanel].basename === 'undefined') {
                ...
                title = file_info[currPanel].basename;
            }
            ...
            exec: function (editor) {
                ...
                if (typeof file_info[currPanel].internalPath !== 'undefined') {
                    ...
                    if (isSaved === true) {
                        ...
                        set_panel_title(currPanel, file_info[currPanel].basename);
                    }
                    else {
        ...
        editor[currPanel].commands.addCommand({
            ...
            exec: function (editor) {
                ...
                var opts = { parent_pid: THIS_PROCESS_NAME, parent_iid: instance, contents: editor.getValue(), ext: true };
                if (typeof file_info[currPanel].internalPath !== 'undefined') {
        ...
        editor[currPanel].commands.addCommand({
            ...
            exec: function (editor) {
                ...
                $dialog.find('fieldset[name="filename"]').children('legend').html(file_info[currPanel].filename);
                $dialog.find('p[name="file_path"]').html('<b>File Path:</b> ' + file_info[currPanel].externalPath);
                $dialog.find('p[name="file_size"]').html('<b>File Size:</b> ' + file_info[currPanel].sizeInOctets);
                $dialog.find('p[name="file_type"]').html('<b>File Type:</b> ' + file_info[currPanel].extension);
    ...
    var set_panel = function (panel_id, path) {
        if (typeof path !== 'undefined') {
            ...
            file_info[current] = file.open(path);

            set_panel_title(current, file_info[current].basename);

            editor[current].setValue(file_info[current].contents);
            _window.setTitle(file_info[current].externalPath + ' - CodeMaster');
            editor[current].getSession().setUndoManager(new ace.UndoManager());
            if (typeof exts[file_info[current].extension] === 'undefined') {
                editor[current].getSession().setMode("ace/mode/text");
            }
            else {
                editor[current].getSession().setMode("ace/mode/" + exts[file_info[current].extension]);
            }
        ...
        if (typeof path === 'undefined') {
            ...
            new FlatOS.System.Application.FileSelectorDialog(opts, function (filepath) {
                if (currentUsed) {
                    ...
                    add_panel(file_info[current].basename);
    ...
    var openLaunch = function (path) {
        var current = panelNB + 1;
        var currentUsed = (typeof file_info[current - 1].internalPath != 'undefined');
        if (currentUsed) {
            openFile(path);
        }
        else {
    ...
    _window.on('update', function () { editor[currPanel].resize(true); });
    _window.on('resizeEnd', function () { editor[currPanel].focus(); });
    _window.on('maximize', function () { editor[currPanel].focus(); });
