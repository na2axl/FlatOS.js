...
(function () {
    ...
    ContentTools.InspectorUI = (function (_super) {
        ...
        InspectorUI.prototype.mount = function () {
            ...
            this.updateCounter();
            this._domViewMode = this.constructor.createDiv(['ct-inspector__viewmode']);
            this._domElement.appendChild(this._domViewMode);
            var app = ContentTools.EditorApp.get();
            if (app.isEditing()) {
                $(this._domViewMode).addClass('editing');
            }
            else {
                $(this._domViewMode).addClass('reading');
            }
            this._domSaving = this.constructor.createDiv(['ct-inspector__saving', 'hide']);
            var p = this.constructor.createDiv(['ct-progress']);
            var b = this.constructor.createDiv(['bar']);
            p.appendChild(b);
            this._domSaving.appendChild(p);
            this._domElement.appendChild(this._domSaving);
            this._addDOMEventListeners();
        ...
        InspectorUI.prototype.toggleViewmode = function () {
            var app = ContentTools.EditorApp.get();
            if (!app.isEditing()) {
                $(this._domViewMode).removeClass('reading');
                $(this._domViewMode).addClass('editing');
                return app.start();
            }
            $(this._domViewMode).removeClass('editing');
            $(this._domViewMode).addClass('reading');
            return app.stop(true);
        };

        InspectorUI.prototype._addDOMEventListeners = function () {
            this._domViewMode.addEventListener('click', (function (_this) {
                return function () {
                    return _this.toggleViewmode();
                };
            })(this));
    ...
    ContentTools.Tools.FontSize = (function (_super) {
        ...
        FontSize.isApplied = function (element, selection) {
            ...
            return element.content.slice(from, to).hasTags(this.tagName, true) && $(HTMLString.String.decode(element.content.slice(from, to).html())).hasClass('fontsize');
        };
        ...
        FontSize.apply = function (element, selection, callback) {
            ...
            element.storeState();
            _ref = selection.get(), from = _ref[0], to = _ref[1];
            ['b', 'u', 'i', 'strike', 'sub', 'sup'].forEach(function (tag) {
                element.content.unformat(from, to, new HTMLString.Tag(tag));
            });
            var $element = $(element._domElement);
            var size = parseInt($element.css('font-size')) + this.factor;
            if (parseInt($(HTMLString.String.decode(element.content.slice(from, to).html())).css('font-size')) > 0) {
                size = parseInt($(HTMLString.String.decode(element.content.slice(from, to).html())).css('font-size')) + this.factor;
            }
            size = (size > 8) ? size : 8;
    ...
    ContentTools.Tools.Unformat = (function (_super) {
        ...
        Unformat.tagNames = ['b', 'i', 'u', 'strike', 'sub', 'sup', 'font', 'span'];
